...
cas_username = os.environ.get("CAS_USERNAME") or username
...
# 登录
# 如果配置了CAS_PASSWORD，先通过CAS+VPN获取webvpn-token，再用带token的cookies登录教务系统
if cas_password:
    from scripts.cas_vpn_login import cas_vpn_login
    print("[*] 检测到CAS_PASSWORD，使用 CAS+VPN 前置认证...")
    vpn_session, cas_err = cas_vpn_login(cas_username, cas_password)
    if vpn_session is None:
        print(f"[-] CAS+VPN认证失败: {cas_err}")
        sys.exit(1)
    # 只提取 webvpn-token 注入，其他CAS cookies不要带入（会和教务系统cookie冲突）
    vpn_token = None
    for c in vpn_session.cookies:
        if c.name == "webvpn-token":
            vpn_token = c.value
            break
    if not vpn_token:
        print("[-] 未能获取 webvpn-token")
        sys.exit(1)
    student_client = login(url, username, password, cookies={"webvpn-token": vpn_token})
else:
