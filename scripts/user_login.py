...
def login(url, username=None, password=None, session=None, cookies=None):
    """
    封装登录逻辑，兼容四种场景：
    1. 直接复用已登录的 requests.Session（跳过教务登录）
    2. 使用现成 cookies（跳过教务登录）
    3. 账号密码登录
    4. 预置 cookies + 账号密码（用于 CAS+VPN 场景：先注入 VPN token 再走教务登录）
    """
    if cookies is None:
        cookies = {}
    ...
    # 若已提供 session 且没有 username/password，直接复用（跳过教务登录）
    if session is not None and username is None:
    ...
    # 若提供了 cookies 但没有 username/password，直接使用（跳过教务登录）
    if cookies and username is None:
        student_client = Client(
            cookies=cookies,
            base_url=base_url,
            raspisanie=raspisanie,
    ...
    # 如果有预置 cookies（如 webvpn-token），注入到 session 中并指定域名
    if cookies:
        for k, v in cookies.items():
            student_client.sess.cookies.set(k, v, domain=".cqytxy.edu.cn")
